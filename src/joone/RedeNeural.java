...
public class RedeNeural implements Serializable {
	...
	public void gerarArquivoTreino() {
		try {
			...
			System.out.println("Exce��o no m�todo gravarNumero(): " + e.getMessage());
		}
	}
	...
	private int trainingPatterns() {
		...
		try {
			...
			System.out.println("Exce��o no m�todo trainingPatterns(): " + e.getMessage());
		}
	...
	public String testar(String charMatriz, TipoCaractere tipo) {
		try {
			...
			System.out.println("Exce��o no m�todo testar: " + e.getMessage());
		}
		JooneOCR joo = JooneOCR.restoreNeuralNet(ocrSerializable);
		joo.interrogate(inputSelector);

		System.out.println("O caractere lido �: " + this.alfabeto.getLetraIndex(joo.caractereIndex(tipo)).toUpperCase());
